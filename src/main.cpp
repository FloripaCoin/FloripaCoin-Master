...
const string strMessageMagic = "Floripacoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	*/
	int BlockCountA = 1000000;
	int BlockCountB = 420000;
	if (nHeight <= 10){ nSubsidy = 36000000 * COIN; }
	else if (nHeight < BlockCountA) {
        if ((nHeight % 1000) < 21)
	    // Reward between 500 and 50000
            nSubsidy = (501 + 49499) * COIN;
        else if (nHeight % 1000 == 21)
            nSubsidy = 50000 * COIN;
    }
	else if (nHeight < BlockCountA * 3) nSubsidy >>= (nHeight / 733394);
	else
		nSubsidy = 600 * COIN;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Floripacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
	static const int64	BlocksTargetSpacing			= 43; // 43 Seconds
...
void ThreadScriptCheck() {
    RenameThread("floripacoin-scriptch");
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xed91ac4a8efa09891e284ac7b02e7c12b8d1a8e3a98e2d21129a494d81939f85");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1395192203; //epochtime
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FloripacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FloripacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FloripacoinMiner(CWallet *pwallet)
{
    printf("FloripacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("floripacoin-miner");
    ...
    try { loop {
        ...
        printf("Running FloripacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FloripacoinMiner terminated\n");
        throw;
    }
}

void GenerateFloripacoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FloripacoinMiner, pwallet));
}
