...
bool RecvLineFixed(SOCKET hSocket, string& strLine)
{
    ...
    loop
    {
		...
		// +floripacoin added for irc exits, without this, recv(hSocket, &c, 1, 0); will hang when closing the program after using irc on windows.
		bool stop = false;
		u_long RecvCount;
        while(!stop)
        {
...
void ThreadIRCSeed()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FL0R1PAC01NTEST3\r");
            Send(hSocket, "WHO #FL0R1PAC01NTEST3\r");
        } else {
            // randomly join #FL0R1PAC01N00-#FL0R1PAC01N99
            int channel_number = GetRandInt(100);
			channel_number = 1; // For now, just use one channel
            Send(hSocket, strprintf("JOIN #FL0R1PAC01N%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FL0R1PAC01N%02d\r", channel_number).c_str());
        }
