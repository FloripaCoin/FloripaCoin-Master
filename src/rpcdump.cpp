...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <floripacoinprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <floripacoinaddress>\n"
            "Reveals the private key corresponding to <floripacoinaddress>.");
    ...
    return CFloripacoinSecret(vchSecret).ToString();
}
