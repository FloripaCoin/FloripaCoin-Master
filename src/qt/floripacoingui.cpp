 ...
 * The Floripacoin Developers 2011-2012
 */
...
const QString FloripacoinGUI::DEFAULT_WALLET = "~Default";

FloripacoinGUI::FloripacoinGUI(bool fIsTestnet, QWidget *parent) :
    QMainWindow(parent),
    ...
    prevBlocks(0)
{
    ...
    if (!fIsTestnet)
    {
        ...
        QApplication::setWindowIcon(QIcon(":icons/floripacoin"));
        setWindowIcon(QIcon(":icons/floripacoin"));
    }
    else
    {
        setWindowTitle(tr("Floripacoin") + " - " + tr("Wallet") + " " + tr("[testnet]"));
        QApplication::setWindowIcon(QIcon(":icons/floripacoin_testnet"));
        setWindowIcon(QIcon(":icons/floripacoin_testnet"));
    }
#else
    ...
    if (!fIsTestnet)
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/floripacoin"));
    else
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/floripacoin_testnet"));
#endif
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void FloripacoinGUI::RunMiningAsStartup()
{
...
void FloripacoinGUI::SetMiningStatus(bool isMining)
{
    if(ui->checkBox)
        ui->checkBox->setChecked(isMining);
}

FloripacoinGUI::~FloripacoinGUI()
{
    //saveWindowGeometry();
...
void FloripacoinGUI::createActions(bool fIsTestnet)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Floripacoin address"));
    ...
    if (!fIsTestnet)
        aboutAction = new QAction(QIcon(":/icons/floripacoin"), tr("&About Floripacoin"), this);
    else
        aboutAction = new QAction(QIcon(":/icons/floripacoin_testnet"), tr("&About Floripacoin"), this);
    ...
    if (!fIsTestnet)
        toggleHideAction = new QAction(QIcon(":/icons/floripacoin"), tr("&Show / Hide"), this);
    else
        toggleHideAction = new QAction(QIcon(":/icons/floripacoin_testnet"), tr("&Show / Hide"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Floripacoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Floripacoin addresses"));
...
void FloripacoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void FloripacoinGUI::createToolBars()
{
...
void FloripacoinGUI::setClientModel(ClientModel *clientModel)
{
...
void FloripacoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void FloripacoinGUI::createTrayIcon()
{
#ifndef Q_OS_MAC
...
void FloripacoinGUI::createTrayIconMenu()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FloripacoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FloripacoinGUI::saveWindowGeometry()
{
    QSettings settings;
...
void FloripacoinGUI::restoreWindowGeometry()
{
    QSettings settings;
...
void FloripacoinGUI::optionsClicked()
{
...
void FloripacoinGUI::aboutClicked()
{
...
void FloripacoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void FloripacoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void FloripacoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
...
void FloripacoinGUI::changeEvent(QEvent *e)
{
...
void FloripacoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void FloripacoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        "which goes to the nodes that process your transaction and helps to support the network. "
        "Do you want to pay the fee?").arg(FloripacoinUnits::formatWithUnit(FloripacoinUnits::MEC, nFeeRequired));
...
//void FloripacoinGUI::incomingTransaction(const QString& date, int unit, qint64 amount, const QString& type, const QString& address)
void FloripacoinGUI::incomingTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(FloripacoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}

void FloripacoinGUI::gotoOverviewPage()
{
...
void FloripacoinGUI::gotoHistoryPage()
{
...
void FloripacoinGUI::gotoAddressBookPage()
{
...
void FloripacoinGUI::gotoReceiveCoinsPage()
{
...
//void FloripacoinGUI::gotoSendCoinsPage(QString addr)
void FloripacoinGUI::gotoSendCoinsPage()
{
...
void FloripacoinGUI::gotoSignMessageTab(QString addr)
{
...
void FloripacoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void FloripacoinGUI::gotoServiceMessagesPage()
{
...
void FloripacoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FloripacoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
	    ...
	    gotoSendCoinsPage();
            ...
            message(tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Floripacoin address or malformed URI parameters."),
...
bool FloripacoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
void FloripacoinGUI::resizeEvent(QResizeEvent *e)
{
    updateMask();
    QMainWindow::resizeEvent(e);
}

void FloripacoinGUI::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
...
void FloripacoinGUI::updateMask()
{
    // main form mask
    _mask = QBitmap(size());
    _mask.clear();
...
void FloripacoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void FloripacoinGUI::menuFileRequested()
{
    QMenu menu(this);
...
void FloripacoinGUI::menuOperationsRequested()
{
    QMenu menu(this);
    QAction* send = menu.addAction(tr("Send Floripacoins"));
    QAction* receive = menu.addAction(tr("Receive Floripacoins"));
...
void FloripacoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FloripacoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void FloripacoinGUI::backupWallet()
{
...
void FloripacoinGUI::changePassphrase()
{
...
void FloripacoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void FloripacoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void FloripacoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FloripacoinGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void FloripacoinGUI::onMiningClicked()
{
    //ui->wMining->setStyleSheet("background-color:black;");
    centralWidget->setCurrentWidget(miningPage);
}

void FloripacoinGUI::on_bHelp_clicked()
{    
    rpcConsole->show();
}
